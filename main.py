...
from sqlmodel import create_engine, Session, SQLModel, Field, select, func
    ...
    with Session(engine) as session:
        if not session.exec(select(Campaign)).first():
            session.add_all([
                ...
                Campaign(name="summer break", due_date= datetime.now(timezone.utc))
            ])
            session.commit()
    yield
...
async def read_campaigns(request: Request, session: SessionDep, page: int = Query(1, ge=1), page_size: int = Query(20, ge=1)):
    limit = page_size #jumlah yang mau ditampilin berapa
    offset = (page -1 ) * limit
